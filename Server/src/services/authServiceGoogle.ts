...
export class AuthServiceGoogle {
    ...
    public async verifyGoogleToken(idToken: string): Promise<GoogleTokenPayload> {
        try {
            ...
            if(!payload?.email || !payload?.given_name || !payload?.family_name || !payload?.email_verified) {
                return Promise.reject(new Error("Informazioni utente mancanti"));
            }

            return {nome: payload.given_name, cognome: payload.family_name, email: payload.email, image: payload.picture};

        } catch (error: any) {
            return Promise.reject(new Error('Errore di verifica del token: ' + error?.message));
        }
    }
}