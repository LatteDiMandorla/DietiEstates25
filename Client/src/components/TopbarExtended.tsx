...
export const TopbarExtended = ({shrink} : {shrink: boolean}) => {
    ...
    return (
        <div className={`flex flex-col w-full items-center justify-end bg-[#DDF5FF] ${shrink ? "h-20 py-0 gap-0"  : "h-64"} transition-all duration-300 relative`}>
            <div className={`flex w-full justify-end p-1 gap-8 transition-all duration-200 absolute top-2 ${shrink && "opacity-0 md:opacity-100"}`}>
                <div className="h-16 flex items-center"><NotificationsMenu /></div>
                {auth && <Avatar size="md" src={auth?.image || ""} />}
            </div>

            {/* Secondo item (centrato) */}
            <div className={`flex w-full h-full rounded-lg absolute transition-all duration-300 ${shrink ? "opacity-0 md:opacity-100 translate-x-1/2 translate-y-1/2 md:translate-x-0 md:translate-y-0" : "opacity-100 translate-x-1/2 translate-y-1/2"}`}>
