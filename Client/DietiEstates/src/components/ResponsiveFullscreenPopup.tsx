...
export const ResponsiveFullscreenPopup: React.FC<ResponsiveFullscreenPopupProps> = ({ children, className = "", open, close}) => {
    return (
        ...
        <div className={`absolute inset-0 md:inset-16 bg-[#FAFAFA] z-[200] max-h-screen md:rounded-md ${open ? "flex flex-col" : "hidden"} overflow-hidden shadow-md select-none ` + className}>
            {children}
        </div>
        </>
    )
